#!/usr/bin/env python3
"""
Overlay a single organ mask on a full-body segmentation and compute 3D volumes, 2D extents,
and 3D centerline length. Generates an interactive 3D HTML viewer and saves measurements to a TXT file.
...
from skimage.morphology import skeletonize   # 3D용 skeletonize 함수 (ND 지원) :contentReference[oaicite:3]{index=3}
...
def compute_3d_volume(vol, header, threshold=0.5):
    ...
    return binary.sum() * voxel_vol  # mm³ 또는 ml :contentReference[oaicite:4]{index=4}
...
def compute_3d_length(vol, header, threshold=0.5):
    binary = vol > threshold
    skeleton = skeletonize(binary)          # ND 골격화 수행 :contentReference[oaicite:5]{index=5}
    spacing = np.mean(header.get_zooms())
    return skeleton.sum() * spacing         # 총 길이(mm)
...
def main():
    ...
    # 뷰어 크기 고정 및 여백 최소화 :contentReference[oaicite:6]{index=6}
    fig.update_layout(
        autosize=False,
        width=800,
        height=600,
    ...
    # Plotly.js를 CDN으로 로드하여 HTML 용량 최소화 :contentReference[oaicite:7]{index=7}
    ...
    length_mask = compute_3d_length(vol_mask, hdr_mask, args.threshold)
    ...
    with open(txt_path, 'w') as f:
        ...
        f.write(f"Mask 3D Length (mm): {length_mask:.2f}\n")
